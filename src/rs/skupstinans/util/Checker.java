...
public class Checker {
	...
	public void checkPropis(List<String> messages, Propis propis) {
		...
		checkString(messages, propis.getObrazlozenje(), "Nedostaje obrazloÅ¾enje za predlaganje propisa");
	}
		...
		if (propis.getClan().isEmpty() && propis.getDeo().isEmpty() && propis.getGlava().isEmpty()) {
			...
			if (propis.getClan().isEmpty() && propis.getGlava().isEmpty()) {
				...
				if (propis.getDeo().size() == 1) {
					messages.add("Propis moÅ¾e da se sastoji od barem 2 dela");
				}
			...
			if (propis.getDeo().isEmpty() && propis.getClan().isEmpty()) {
				if (propis.getDeo().size() == 1) {
					messages.add("Propis moÅ¾e da se sastoji od barem 2 glave");
				}
	...
	public void checkGroup(List<String> messages, Object group, String messagePrefix) {
		...
		if (element.getContent().isEmpty()) {
			messages.add(String.format("%s - nedostaje sadrÅ¾aj", messagePrefix));
		} else {
			...
			for (int i = 0; i < element.getContent().size(); i++) {
				...
				if (obj instanceof Clan) {
					...
					checkClan(messages, (Clan) obj, String.format("%s ÄŒlan %s.", messagePrefix, clanCounter),
							innerClanCounter == 1);
			...
			for (Class cl : viewIndexes.keySet()) {
				...
				default:
					if (viewIndexes.get(cl) == 1) {
						messages.add(String.format("%s moÅ¾e da se deli na barem 2 podelementa tipa %s", messagePrefix,
								cl.getSimpleName()));
					}
				}
			}
